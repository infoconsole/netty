    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            if (eventLoop.inEventLoop()) {
                ...
                try {
                    //每次调用完成监听处理以后  会处理tasj
                    Runnable regRun = new Runnable() {
                        @Override
                        public void run() {
                            register0(promise);
                        }
                    };
                    eventLoop.execute(regRun);
                } catch (Throwable t) {
                    logger.warn(
        ...
        private void register0(ChannelPromise promise) {
            System.out.println("EventLoop thread is " + Thread.currentThread().getName() + "can register0");
            try {
                ...
                System.out.println("注册初始化以后的pipeline是" + pipeline.toString());
